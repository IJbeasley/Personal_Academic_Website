...
If you can't work this out alone, check out the [wikipedia page on the binomial distribution](https://en.wikipedia.org/wiki/Binomial_distribution) to try to figure these values out for yourself before checking the answers below.
...
Now, let's expand our previous plot where only considered one possible trajectory of allele frequency changes over generations to now consider 10 possible trajectories (i.e, the code output for 10 different simulations written for the previous question).
...
Can you use the `rep_wf_n_gen` function to perform 10 simulations across 50 generations for populations with initial A allele frequency of 20%? Use this function to simulate across several different population sizes (5, 10, 100, 10,000, 1,000,000).  
...
Next, can you use the `rep_wf_n_gen` function to perform 10 simulations across 50 generations, for populations consisting of 100 individuals? For this question, vary the initial allele frequency (0.05,0.1,0.2,0.3,0.5). 
